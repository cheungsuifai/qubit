...
if __name__ == '__main__':
    ...
    while not stop:
        ...
        type = parse_msg(input_string)
        if type == 'alert':
            ne_type = parse_ne_type(input_string)

            if ne_type == 'PG':
                print('Handling PG alert...')
                resut_dict = parse_alert_result(input_string)
                print('系统判断故障概率分布为：',resut_dict)
                input_string = input("请输入类型：")

                if input_string in ['PG','HLR','HSS','CUDB','UserData','Boss']:
                    print(ops_dict[input_string])
